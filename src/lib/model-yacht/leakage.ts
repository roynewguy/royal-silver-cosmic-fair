...
/** MLB games are not known-final at first pitch. Require a buffer before using a prior result. */
export const PRIOR_COMPLETE_MS = 3.5 * 3_600_000;

export function yachtPriorGames(
  all: HistoricalGame[],
  teamAbbr: string,
  predictionAt: string,
  completeMs = PRIOR_COMPLETE_MS,
): HistoricalGame[] {
