...
import { alertOwner, resultDeliveryAlertCode } from "./alerts";
...
export async function flushResultRecaps(): Promise<void> {
  ...
  if (stale.length) await alertOwner("DISCORD_DELIVERY_UNKNOWN", "Result delivery uncertain; review result feed before any resend.");
  ...
  for (const row of rows) {
    ...
    try {
      ...
      if (!sent.ok) {
        ...
        await alertOwner(
          resultDeliveryAlertCode(sent),
          sent.uncertain
            ? "Result recap delivery unknown; do not resend."
            : "Result recap failed; inspect delivery status.",
        );
        continue;
      }
      ...
      await alertOwner("DISCORD_DELIVERY_UNKNOWN", "Result confirmation could not be stored; do not resend.");
    }
  }
}