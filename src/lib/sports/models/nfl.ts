...
function weatherLean(weather: string | null): { under: number; note: string } {
  if (!weather) return { under: 0, note: "" };
  const w = weather.toLowerCase();
  const temp = Number((w.match(/(-?\d+)\s*°/) ?? [])[1]);
  const windy = /wind|gust/.test(w);
  const wet = /rain|snow|shower/.test(w);
  let under = 0;
  if (Number.isFinite(temp) && temp <= 32) under += 0.04;
  if (windy) under += 0.03;
  if (wet) under += 0.03;
...
export function rankNfl(game: GameCard): RankPick | null {
  ...
  const wx = weatherLean(game.weather);
  const qb = [
    ...injuryNotes(game, "home"),
    ...injuryNotes(game, "away"),
  ];
