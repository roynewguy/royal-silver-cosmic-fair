...
test("official LOCK embed matches BetStars-style card shape with 1u", () => {
  ...
  assert.match(embed.description ?? "", /^NBA\nWarriors vs Lakers/m);
  assert.match(embed.description ?? "", /\*\*PICK\*\*/);
  assert.match(embed.description ?? "", /\*\*Lakers -3\.5 -110\*\*/);
  assert.match(embed.description ?? "", /Sportsbook: DraftKings/);
  assert.match(embed.description ?? "", /Posted:/);
  assert.match(embed.description ?? "", /Ticket: BB-9/);
  assert.doesNotMatch(embed.description ?? "", /WHY BoatBoyzPicks LIKES IT/);
  assert.doesNotMatch(embed.description ?? "", /No-Vig|no-vig|PASS_/);
  ...
  assert.equal(officialTierBadgePlain(pick), "🚨 BOATBOYZ LOCK");

  const names = (embed.fields ?? []).map((f) => f.name);
  assert.deepEqual(names, ["Market", "Line", "Odds", "Units", "Model", "Kick"]);
  const byName = Object.fromEntries((embed.fields ?? []).map((f) => [f.name, f.value]));
  assert.equal(byName.Market, "Spread");
  assert.equal(byName.Line, "-3.5");
  assert.equal(byName.Odds, "-110");
  assert.equal(byName.Units, "1u");
  assert.equal(byName.Model, "V2");
  assert.match(byName.Kick!, /PT$/);
  assert.match(embed.footer?.text ?? "", /^BoatBoyz · BB-9 · /);
...
test("official soft-floor embed is BEST AVAILABLE / DESK PICK with 0.5u desk and never LOCK", () => {
  ...
  const embed = buildOfficialPickEmbed(pick, game);
  assert.equal(resolvePickTier(pick), "soft_floor");
  assert.equal(officialTierBadgePlain(pick), "BOATBOYZ DESK");
  assert.equal(unitsFieldLabel(pick), "0.5u desk");
  assert.match(embed.author?.name ?? "", /BOATBOYZ DESK/);
  ...
  assert.equal(units, "0.5u");
  assert.match(embed.description ?? "", /NFL/);
  assert.match(embed.description ?? "", /Broncos vs Seahawks/);
  assert.match(embed.description ?? "", /\*\*PICK\*\*/);
  assert.match(embed.description ?? "", /SEA -3 -110/);
  assert.doesNotMatch(embed.description ?? "", /WHY BoatBoyzPicks LIKES IT/);
  assert.equal(embed.color, OFFICIAL_EMBED_COLOR);
});
...
test("LOCK vs soft-floor embed badges and units differ on the same slate", () => {
  ...
  assert.match(lockEmbed.author?.name ?? "", /🚨 BOATBOYZ LOCK/);
  assert.match(softEmbed.author?.name ?? "", /BOATBOYZ DESK/);
  assert.doesNotMatch(softEmbed.author?.name ?? "", /\bLOCK\b/);
  assert.doesNotMatch(JSON.stringify(softEmbed), /\bLOCK\b/);
  assert.equal(lockEmbed.fields?.find((f) => f.name === "Units")?.value, "1u");
  assert.equal(softEmbed.fields?.find((f) => f.name === "Units")?.value, "0.5u");
...
test("official result embed is gold-bar WIN/LOSS/PUSH with clean fields and never LOCK", () => {
  ...
  assert.match(winEmbed.author?.name ?? "", /BoatBoyz/);
  ...
  assert.match(winEmbed.description ?? "", /\*\*PICK\*\*/);
  assert.match(winEmbed.description ?? "", /ATH ML \+144/);
  const names = (winEmbed.fields ?? []).map((f) => f.name);
  assert.deepEqual(names, ["Result", "P/L", "Record", "Frozen odds", "Final", "Ticket", "Kick"]);
  const byName = Object.fromEntries((winEmbed.fields ?? []).map((f) => [f.name, f.value]));
  assert.equal(byName.Result, "✅ WIN");
  assert.equal(byName["P/L"], "+2.88u");
  assert.equal(byName.Record, "3-1-0 · +3.80u · ROI 76.0%");
  assert.equal(byName["Frozen odds"], "+144");
  assert.equal(byName.Final, "Final TOR 0 @ ATH 2");
  assert.equal(byName.Ticket, "BB-42");
  assert.match(byName.Kick!, /PT$/);
  assert.match(winEmbed.footer?.text ?? "", /original \+144/);
  ...
  assert.match(pushEmbed.author?.name ?? "", /➖ PUSH/);
