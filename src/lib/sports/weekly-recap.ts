...
export function buildWeeklyRecap(period: {start:string;end:string}, week: DeskRecord & {voids:number}, overall: DeskRecord): string {
  const sunday = new Date(`${period.end}T12:00:00Z`); sunday.setUTCDate(sunday.getUTCDate()-1);
  return ["🌊 **BOATBOYZ • WEEKLY RECAP**", `📅 ${period.start} – ${sunday.toISOString().slice(0,10)} • Pacific Time`, "",
    ...
    "Every result recorded. No losses removed."].join("\n");
}
