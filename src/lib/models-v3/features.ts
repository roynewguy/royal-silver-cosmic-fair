...
import type { HistoricalGame, HistoricalOdds, StarterFeat, TeamFeat, TrainingRow } from "./types.ts";
...
export function teamFeatures(priors: HistoricalGame[], abbr: string, startAt: string, minPrior = 10): TeamFeat | null {
  assertNoFutureGames(priors, startAt);
  if (priors.length < minPrior) return null;
  ...
  return {
    ...
    runDiffPg: ordered.reduce((s, g) => s + (runsFor(g, abbr) - runsAgainst(g, abbr)), 0) / ordered.length,
    restDays,
  };
}
...
export function featureVector(row: TrainingRow): number[] {
...
export function buildRows(
  ...
  for (const game of sorted) {
    if (game.status !== "final" || game.homeScore == null || game.awayScore == null || game.homeScore === game.awayScore) {
      dropped += 1;
      continue;
    }
    const home = teamFeatures(priorGames(sorted, game.homeAbbr, game.startAt), game.homeAbbr, game.startAt, minPrior);
    const away = teamFeatures(priorGames(sorted, game.awayAbbr, game.startAt), game.awayAbbr, game.startAt, minPrior);
    if (!home || !away) {
      dropped += 1;
      continue;
    }
