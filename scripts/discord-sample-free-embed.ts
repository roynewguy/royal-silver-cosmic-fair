#!/usr/bin/env node
/**
 * One-shot: post 1 TEST FREE sample embed to DISCORD_TEST_WEBHOOK only.
 * Never touches picks/results/alerts webhooks. No DB / ledger writes.
 *
 *   DISCORD_TEST_WEBHOOK=https://discord.com/api/webhooks/... \
 *     node --experimental-strip-types scripts/discord-sample-free-embed.ts
 */
import {
...
async function main() {
  ...
  const samples: Array<{ label: string; pick: PickRow; game: GameCard }> = [
    { label: "SAMPLE FREE · LOCK", pick: lockPick, game: lockGame },
  ];
  for (const sample of samples) {
    const payload = buildOfficialPickPayload(sample.pick, sample.game);
    const body = {
      content: `🧪 **TEST FREE SAMPLE — NOT A LIVE FREE PICK** · ${sample.label}`,
  ...
  console.log("Done. One FREE TEST sample posted to DISCORD_TEST_WEBHOOK only.");
}
